...
impl Frame {
    ...
    fn maybe_shoot_two(&self) -> Option<u16> {
        self.shoot2.or_else(|| self.bonus.one)
    }
...
impl BowlingGame {
    ...
    pub fn roll(&mut self, pins: u16) -> Result<(), Error> {
        ...
        println!("{:?}", maybe_in_progress_frame);
...
#[cfg(test)]
mod tests {
    use crate::{ExtraRoll, Frame};

    #[test]
    fn frame_is_completed() {
        let open = Frame::new(0, false);
        assert!(!open.is_completed());
        let strike = Frame::new(10, false);
        assert!(strike.is_completed());

        let mut spare = Frame::new(3, false);
        let _ = spare.add_shoot(6);
        println!("{:?}", spare);
        assert!(spare.is_completed());
    }

    #[test]
    fn extra_roll_valid_score() {
        let mut e = ExtraRoll::new();
        assert!(e.is_valid_score());
        let _ = e.push(10);
        assert!(e.is_valid_score());
        let _ = e.push(5);
        println!("{:?}", e);
